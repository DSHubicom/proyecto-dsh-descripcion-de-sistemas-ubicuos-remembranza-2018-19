...
void switchCase (char command){
   switch (command) {
    ...
    case 'w': {
        ...
        uint16_t statuscode;
        int16_t length;
        String url = "destination-api-remembranza.herokuapp.com/J985/Service_01_Destination_API/1.0.0/destination/1";
        ...
        while (idx != -1){
            ...
            Serial.println(arg);
            //Get position where instruction ends
            int idx2= arg.indexOf("[");
            String x_position = arg.substring(0, idx2);
            String x_position = arg.substring(1, idx2);
            ...
            String x="";
            String y="";
            while(x!=x_position && y!=y_position){
                Speech(arg);
                for(int i=0; i< sizeof(gpsdata); i++){
                  if(index == 3)
                    x= x + gpsdata[i];
                ...
                //If position is out of permited range message is sent
                if(x > 36.4864846 || y > -6.1184475){
                    char sendto[21]= ""; 
                    String message ="Salida en long: " + x + " y lat: " + y;
                    char sms[141]; 
                    message.toCharArray(sms,141);
